...
export const getImages = (limit: number = 10) => async (dispatch: AppDispatch) => {
    try {
        dispatch(CatSlice.actions.imagesFetching());
        const response = await axios.get<ICat[]>(`https://api.thecatapi.com/v1/images/search?limit=${limit}`, {
            headers: {
                'x-api-key': 'live_7K0GEoWfeVTF4ITXkvrLo4RlzMKqNKqbSQHAxnHQAW72T4f4OUvCiZuNJtIYagXf',
            }
        });
